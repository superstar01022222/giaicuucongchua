...
void event_info(string s)



void event1(knight& knight, int order){
	int evelO = order > 6 ? (order % 10 > 5 ? order % 10 : 5) : order % 10;
...
void event7(knight& knight, int order){
	if (knight.check_frog||knight.check_tiny){
		knight.end_event();
		return;
		levelO = order > 6 ? (order % 10 > 5 ? order % 10 : 5) : order % 10;
	if (knight.level>levelO){
		knight.level=min(10, knight.level+2);
		else if (knight.level<levelO){
			knight.is_frog=true;
...
void event19(knight& knight){ //asclepius
	ifstream file;
	...
	knight.end_event();
	knight.asclepius=true;  //next time not meet
	file.close();
}
...
struct mushghost{
	void data_mushghost(string file_mush_ghost){
		ifstream file;
		int n;
